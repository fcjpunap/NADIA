...
class MensajesController extends Controller {
    ...
    public function __construct() {
        ...
        if (!isset($_SESSION['user_id'])) { header('Location: ' . URL_BASE . 'auth/login'); exit; }
        $this->db = (new Database())->connect();
        $this->db->query("DELETE FROM mensaje_destinatarios WHERE carpeta='Eliminado' AND borrado_papelera_at < DATE_SUB(NOW(), INTERVAL 30 DAY)");
    }
    public function index() {
        ...
        if ($carpeta == 'Enviados') {
             // FIX V95: Obtener nombres de destinatarios concatenados
            $sql = "SELECT m.*, 
                    ...
                    FROM mensajes m WHERE m.id_remitente=$uid AND m.estado='Enviado' ORDER BY m.fecha_envio DESC";
        } else {
            $sql = "SELECT m.*, md.leido, md.id as id_relacion, u.nombres as rem_n, u.apellidos as rem_a,
                   (SELECT COUNT(*) FROM mensaje_adjuntos WHERE id_mensaje=m.id) as adjuntos
                    FROM mensaje_destinatarios md 
                    JOIN mensajes m ON md.id_mensaje=m.id
                    JOIN usuarios u ON m.id_remitente=u.id
                    WHERE md.id_destinatario=$uid AND md.carpeta='$carpeta' ORDER BY m.fecha_envio DESC";
        }
    ...
    public function crear() {
        ...
        $rol = $_SESSION['rol'];
        $destinatarios = [];
        // ... Lógica de destinatarios idem V94 ...
        if ($rol == 1) { // Tesista
            ...
            foreach($proyectos as $p) {
             ...
             $sqlA = "SELECT t.id, t.nombres, t.apellidos, p.titulo FROM proyectos p JOIN usuarios t ON p.id_tesista=t.id WHERE p.id_asesor=$uid";
             foreach($this->db->query($sqlA)->fetchAll() as $r) $destinatarios[$r['id']] = "Tesista (Asesorado): {$r['nombres']} {$r['apellidos']}";
             $sqlJ = "SELECT t.id, t.nombres, t.apellidos, p.titulo FROM jurado_asignaciones ja JOIN proyectos p ON ja.id_proyecto=p.id JOIN usuarios t ON p.id_tesista=t.id WHERE ja.id_jurado=$uid";
             ...
             $all = $this->db->query("SELECT id, nombres, apellidos, id_rol_principal FROM usuarios LIMIT 200")->fetchAll();
             foreach($all as $r) $destinatarios[$r['id']] = "Usuario: {$r['nombres']} {$r['apellidos']}";
        }
        $this->view('mensajes/crear', ['destinatarios' => $destinatarios]);
    }
    public function guardar() {
        if($_POST) {
            ...
            if (!empty($_FILES['adjuntos']['name'][0])) {
                ...
                if (!file_exists($uploadDir.'index.html')) file_put_contents($uploadDir.'index.html', '');
            ...
            // FIX V95: Construcción de URL Absoluta
            // Detectar protocolo
            $protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? "https://" : "http://";
            $domainName = $_SERVER['HTTP_HOST'];
            // URL_BASE suele ser '/path/to/app/', así que concatenamos
            $fullUrl = $protocol . $domainName . URL_BASE . 'mensajes/detalle?id=' . $msgId;
            foreach($dests as $did) {
                $this->db->prepare("INSERT INTO mensaje_destinatarios (id_mensaje, id_destinatario) VALUES (?, ?)")->execute([$msgId, $did]);
                if($notificar) {
                    $u = $this->db->query("SELECT email, nombres FROM usuarios WHERE id=$did")->fetch();
                    if($u && $u['email']) {
                         Mailer::enviar($u['email'], 
                                        "Nuevo Mensaje: $asunto", 
                                        "Hola {$u['nombres']},<br>Tienes un nuevo mensaje en NADIA.<br><br>Asunto: $asunto<br><br>Para ver el mensaje y descargar adjuntos ingresa aquí: <a href='$fullUrl'>$fullUrl</a>");
                    }
                }
            }
            header('Location: ' . URL_BASE . 'mensajes/index?msg=Enviado');
        }
    }
    
    // detalle(), descargar(), eliminar() se mantienen igual a V94, 
    // pero incluimos todo el file para evitar errores de corte.
    ...
    public function detalle() {
        ...
        if ($rel && !$rel['leido']) $this->db->prepare("UPDATE mensaje_destinatarios SET leido=1, fecha_lectura=NOW() WHERE id_mensaje=? AND id_destinatario=?")->execute([$id, $uid]);
        $adjs = $this->db->query("SELECT * FROM mensaje_adjuntos WHERE id_mensaje=$id")->fetchAll(PDO::FETCH_ASSOC);
        $this->view('mensajes/detalle', ['m' => $msg, 'adjuntos' => $adjs, 'destinatarios' => $destinatarios, 'soy_remitente' => $soy_remitente]);
    }
    ...
    public function descargar() {
        ...
        if(!file_exists($ruta_fisica)) die("El archivo físico no existe.");
    ...
    public function eliminar() {
        $id = $_GET['id'];
        $uid = $_SESSION['user_id'];
        $this->db->prepare("UPDATE mensaje_destinatarios SET carpeta='Papelera', borrado_papelera_at=NOW() WHERE id_mensaje=? AND id_destinatario=?")->execute([$id, $uid]);
